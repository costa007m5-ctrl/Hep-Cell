...
// Importa os novos componentes de fluxo de pagamento
...
// Define os possíveis passos do fluxo de pagamento
...
// ... (Ícones e Mapeamento de Status permanecem os mesmos) ...
...
const InvoiceItem: React.FC<{ invoice: Invoice; onPay?: (invoice: Invoice) => void; onDetails?: (invoice: Invoice) => void; discountInfo: ReturnType<typeof getDiscountInfo> | null }> = ({ invoice, onPay, onDetails, discountInfo }) => {
    ...
    return (
        <div className="flex items-center justify-between p-4 bg-white dark:bg-slate-800 rounded-xl shadow-sm hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-200">
            <div className="flex items-center space-x-4"><div className={`flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center ${config.bgColor}`}>{config.icon}</div><div><p className="font-semibold text-slate-800 dark:text-slate-100">{invoice.month}</p><p className="text-sm text-slate-500 dark:text-slate-400">{invoice.status === 'Em aberto' ? `Vence em ${formattedDueDate}` : config.label}</p></div></div>
            <div className="text-right">
                {discountInfo && invoice.status === 'Em aberto' ? (<><p className="font-semibold text-green-600 dark:text-green-400">{discountInfo.discountedAmount.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p><p className="text-xs text-slate-500 dark:text-slate-400 line-through">{invoice.amount.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p></>) : (<p className={`font-semibold ${config.textColor}`}>{invoice.amount.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</p>)}
                {invoice.status === 'Em aberto' && onPay && (<button onClick={() => onPay(invoice)} className="text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:underline focus:outline-none mt-1">{discountInfo ? (<span className="flex items-center justify-end gap-1"><span>Antecipar com</span><span className="font-bold bg-green-100 dark:bg-green-900/40 text-green-600 dark:text-green-300 px-1.5 py-0.5 rounded-full text-xs">{discountInfo.discountPercentage.toFixed(0)}% OFF</span></span>) : 'Pagar'}</button>)}
                {invoice.status === 'Boleto Gerado' && onDetails && (<button onClick={() => onDetails(invoice)} className="text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:underline focus:outline-none mt-1">Ver Boleto</button>)}
            </div>
        </div>
    );
};
...
const PageFaturas: React.FC<PageFaturasProps> = ({ mpPublicKey }) => {
    ...
    if (errorInfo) return (
        <div className="w-full max-w-md p-4 space-y-4 animate-fade-in">
            ...
            <div className="p-4 bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                <h3 className="font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M18 8a6 6 0 01-7.743 5.743L10 14l-1 1-1 1H6v2H2v-4l4.257-4.257A6 6 0 1118 8zm-6-4a1 1 0 100 2 1 1 0 000-2z" clipRule="evenodd" /></svg>Análise da IA</h3>
                {errorInfo.isDiagnosing && <div className="flex items-center space-x-2 mt-2"><LoadingSpinner /><p className="text-sm text-slate-500 dark:text-slate-400">Analisando o problema...</p></div>}
                {errorInfo.diagnosis && <div className="mt-2 text-slate-600 dark:text-slate-300 space-y-2 text-sm">{errorInfo.diagnosis.split('\n').map((line, index) => line.startsWith('### ') ? <h4 key={index} className="font-bold text-base text-slate-800 dark:text-slate-100 pt-2">{line.replace('### ', '')}</h4> : <p key={index}>{line}</p>)}</div>}
            </div>
    ...
    const openInvoices = invoices.filter(inv => inv.status === 'Em aberto' || inv.status === 'Boleto Gerado');
    const paidInvoices = invoices.filter(inv => inv.status === 'Paga' || inv.status === 'Expirado' || inv.status === 'Cancelado');
        ...
        <div className="w-full max-w-md space-y-6 animate-fade-in">
            <section aria-labelledby="summary-title">
                ...
                {totalDue > 0 ? (
                     <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg p-6 text-center">
                        ...
                        <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">{invoices.filter(i => i.status === 'Em aberto').length} {invoices.filter(i => i.status === 'Em aberto').length === 1 ? 'fatura pendente' : 'faturas pendentes'}</p>
                    </div>
                ) : (
            ...
            {openInvoices.length > 0 && (
                <section className="space-y-3" aria-labelledby="open-invoices-title">
                    <h2 id="open-invoices-title" className="text-xl font-bold text-slate-800 dark:text-slate-200 px-1">Faturas Pendentes</h2>
                    {openInvoices.map(invoice => {
                         const discountInfo = invoice.status === 'Em aberto' ? getDiscountInfo(invoice.due_date, invoice.amount) : null;
                         return <InvoiceItem key={invoice.id} invoice={invoice} onPay={handlePayClick} onDetails={(inv) => { setSelectedInvoice(inv); setPaymentStep('boleto_details'); }} discountInfo={discountInfo} />;
                    })}
                </section>
            )}
            {paidInvoices.length > 0 && (
                 <section className="space-y-3" aria-labelledby="history-title">
                    <button onClick={() => setIsHistoryExpanded(!isHistoryExpanded)} className="w-full flex justify-between items-center text-left text-xl font-bold text-slate-800 dark:text-slate-200 px-1" aria-expanded={isHistoryExpanded}>
                        <span>Histórico</span><span className={`transform transition-transform duration-200 ${isHistoryExpanded ? 'rotate-180' : 'rotate-0'}`}><ChevronDownIcon /></span>
                    </button>
                     {isHistoryExpanded && <div className="space-y-3 animate-fade-in">{paidInvoices.map(invoice => <InvoiceItem key={invoice.id} invoice={invoice} discountInfo={null} />)}</div>}
                </section>
            )}
        </div>
    );
