...
const BoletoDetails: React.FC<BoletoDetailsProps> = ({ invoice, onBack }) => {
    const [copyButtonText, setCopyButtonText] = useState('Copiar Código de Barras');

    const handleCopy = () => {
        if (invoice.boleto_barcode) {
            navigator.clipboard.writeText(invoice.boleto_barcode);
            setCopyButtonText('Copiado!');
            setTimeout(() => setCopyButtonText('Copiar Código de Barras'), 2000);
        }
    };
    ...
    return (
        <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl shadow-lg transform transition-all animate-fade-in">
            <div className="text-center p-6 sm:p-8 border-b border-slate-200 dark:border-slate-700">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Detalhes do Boleto</h2>
                <p className="text-slate-500 dark:text-slate-400 mt-1">
                    Fatura de {invoice.month} - <strong>{invoice.amount.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</strong>
                </p>
                <span className="mt-2 inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300">
                    {invoice.status}
                </span>
            </div>
            ...
            <div className="p-6 sm:p-8 space-y-6">
                 <p className="text-center text-sm text-slate-600 dark:text-slate-400">
                    Use as opções abaixo para pagar seu boleto. A confirmação pode levar até 2 dias úteis.
                </p>
                ...
                {invoice.boleto_barcode && (
                     <div className="w-full">
                        <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 text-center">Código de Barras (Copia e Cola)</label>
                        <div className="relative p-3 bg-slate-100 dark:bg-slate-900/50 rounded-md">
                            <p className="text-xs text-left break-all text-slate-600 dark:text-slate-300 font-mono text-center">
                                {invoice.boleto_barcode}
                            </p>
                        </div>
                        <button onClick={handleCopy} className="mt-2 w-full flex justify-center py-2 px-4 border border-slate-300 dark:border-slate-600 rounded-md text-sm font-medium text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-700 hover:bg-slate-50 dark:hover:bg-slate-600">
                            {copyButtonText}
                        </button>
                    </div>
                )}
