...
const StoreCarousel: React.FC<StoreCarouselProps> = ({ onBannerClick, variant = 'hero', bannersData }) => {
    ...
    useEffect(() => {
        ...
        const fetchBanners = async () => {
            ...
            } catch (error) { setBanners(fallbackBanners); }
        };
        fetchBanners();
    }, [bannersData]);
    ...
    // Handler seguro que verifica se a prop existe
    ...
    if (variant === 'slim') {
        ...
        return (
            <div className="px-4 py-4 animate-fade-in">
                <div className="relative w-full h-24 sm:h-32 rounded-2xl overflow-hidden shadow-md cursor-pointer group" onClick={() => handleBannerClick(banner.link)}>
                    <div className="absolute inset-0 bg-black/20 group-hover:bg-transparent transition-colors z-10"></div>
                    <img src={banner.image_url} className="w-full h-full object-cover object-center transform group-hover:scale-105 transition-transform duration-1000" alt="Promo" />
                    <div className="absolute inset-y-0 left-0 flex flex-col justify-center px-6 z-20 bg-gradient-to-r from-black/80 via-black/20 to-transparent w-3/4">
                        <h3 className="text-white font-black text-lg uppercase italic tracking-wide drop-shadow-md">{banner.prompt}</h3>
                        {banner.subtitle && <p className="text-white/80 text-xs font-medium">{banner.subtitle}</p>}
                    </div>
                </div>
            </div>
        );
            ...
            <div className="relative w-full aspect-[2/1] sm:aspect-[2.5/1] overflow-hidden rounded-[2rem] shadow-2xl shadow-indigo-500/20 group cursor-pointer">
                <div className="flex transition-transform duration-700 ease-in-out h-full" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
                    {banners.map((banner, index) => (
                        ...
                        >
                             ...
                             <div className="absolute top-4 right-4 z-20 opacity-40 scale-75"><Logo className="h-8 w-8 text-white" variant="light" /></div>
                             <div className="absolute bottom-0 left-0 right-0 p-6 z-20 flex flex-col items-start">
                                <div className="backdrop-blur-md bg-white/10 border border-white/20 rounded-2xl px-4 py-2 mb-1 shadow-lg">
                                    <h3 className="text-white font-black text-xl sm:text-2xl tracking-tight drop-shadow-md">{banner.prompt || 'Oferta Especial'}</h3>
                                </div>
                                {banner.subtitle && <p className="text-slate-200 text-xs sm:text-sm font-medium pl-1 drop-shadow-md">{banner.subtitle}</p>}
                             </div>
                        </div>
                    ))}
                </div>
                {banners.length > 1 && (
                    <div className="absolute bottom-6 right-6 flex space-x-1.5 z-30">
                        {banners.map((_, index) => (
                            <button key={index} onClick={(e) => { e.stopPropagation(); setCurrentIndex(index); }} className={`h-1.5 rounded-full transition-all duration-300 shadow-sm ${currentIndex === index ? 'w-6 bg-white' : 'w-1.5 bg-white/40 hover:bg-white/60'}`} />
                        ))}
                    </div>
                )}
            </div>
